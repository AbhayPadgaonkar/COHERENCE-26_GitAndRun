...
async def get_fund_flow_path(scheme_id: int):
...
async def get_total_transferred(scheme_id: int):
...
async def get_status_distribution(scheme_id: int):
...
async def detect_bottlenecks(scheme_id: int):
