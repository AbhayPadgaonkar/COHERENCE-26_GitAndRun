...
export default function DistrictFundFlowTracker({ schemes = [], districtName = "Mumbai", refreshTrigger = 0 }) {
  const [fundFlows, setFundFlows] = useState<any[]>([])
  const [totalTransferred, setTotalTransferred] = useState(0)
  const [loading, setLoading] = useState(true)
  const [selectedScheme, setSelectedScheme] = useState<string | null>(null)
  const [viewMode, setViewMode] = useState<"incoming" | "beneficiary" | "all">("all")

  useEffect(() => {
    if (schemes.length > 0) {
      setSelectedScheme((schemes[0] as any).scheme_code || (schemes[0] as any).id)
    }
  }, [schemes])
  ...
  // Filter flows based on district perspective
  const filterFlows = () => {
    if (viewMode === "incoming") {
      // Flows coming TO district (from State or Central)
      return fundFlows.filter(flow => {
        const toDistrict = flow.to_level?.toLowerCase() === "district" && 
                         flow.to_entity_name?.toLowerCase().includes(districtName.toLowerCase())
        const fromStateOrCentral = flow.from_level?.toLowerCase() === "state" || 
                                   flow.from_level?.toLowerCase() === "central"
        return toDistrict || (fromStateOrCentral && flow.to_entity_name?.toLowerCase().includes(districtName.toLowerCase()))
      })
    } else if (viewMode === "beneficiary") {
      // Flows going FROM district (to Beneficiaries)
      return fundFlows.filter(flow => {
        const fromDistrict = flow.from_level?.toLowerCase() === "district" && 
                           flow.from_entity_name?.toLowerCase().includes(districtName.toLowerCase())
        const toBeneficiary = flow.to_level?.toLowerCase() === "beneficiary"
        return fromDistrict || toBeneficiary
      })
    }
    // For 'all' view, show flows where district is either sender or receiver
    return fundFlows.filter(flow => {
      const isReceiver = flow.to_level?.toLowerCase() === "district" && 
                        flow.to_entity_name?.toLowerCase().includes(districtName.toLowerCase())
      const isSender = flow.from_level?.toLowerCase() === "district" && 
                      flow.from_entity_name?.toLowerCase().includes(districtName.toLowerCase())
  ...
  // Calculate incoming flows (State/Central → District)
  ...
  // Calculate beneficiary flows (District → Beneficiary)
  const beneficiaryFlows = fundFlows.filter(flow => {
    const fromDistrict = flow.from_level?.toLowerCase() === "district" && 
                       flow.from_entity_name?.toLowerCase().includes(districtName.toLowerCase())
    const toBeneficiary = flow.to_level?.toLowerCase() === "beneficiary"
    return fromDistrict || toBeneficiary
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-[#000080] flex items-center">
            ...
            District Fund Flow Tracker
          </h2>
          <p className="text-sm text-gray-500 mt-1">
            Monitor funds received from State and distributed to Beneficiaries
          </p>
        </div>
        ...
        {schemes.length > 0 && (
          <select
            ...
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#000080]"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-gradient-to-br from-[#000080] to-[#000060] rounded-xl p-4 text-white">
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-4 text-white">
          ...
          <h3 className="text-2xl font-bold">
            {formatCurrency(
              incomingFlows.reduce((sum, flow) => sum + (parseFloat(flow.amount) || 0), 0)
            )}
          </h3>
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-4 text-white">
          ...
          <h3 className="text-2xl font-bold">
            {formatCurrency(
              beneficiaryFlows.reduce((sum, flow) => sum + (parseFloat(flow.amount) || 0), 0)
            )}
          </h3>
          <p className="text-xs opacity-75 mt-2">{beneficiaryFlows.length} beneficiary payments</p>
        </div>
      </div>
      ...
      <div className="flex space-x-2 mb-4 border-b border-gray-200">
        ...
        >
          Beneficiary Payments ({beneficiaryFlows.length})
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          ...
          <p className="text-sm text-gray-500 mt-1">
            ...
            {viewMode === "incoming" && "No incoming transfers from State/Central"}
            {viewMode === "beneficiary" && "No beneficiary payments recorded"}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex items-center space-x-2 mb-3">
                ...
                <div className="flex-1 bg-green-50 rounded-lg p-2 border border-green-200">
                  ...
                  <p className="font-semibold text-[#000080] text-sm">{flow.to_entity_name}</p>
              ...
              <div className="flex items-center justify-between border-t border-gray-100 pt-3">
                <div>
                  <p className="text-2xl font-bold text-[#000080]">{formatCurrency(parseFloat(flow.amount) || 0)}</p>
